...
def get_columns(invoice_list, additional_table_columns):
	...
	if invoice_list:
		income_accounts = frappe.db.sql_list(
			...
			from `tabSales Invoice Item` where docstatus and parent in (%s)
		...
		tax_accounts = frappe.db.sql_list(
			...
			and docstatus and parent in (%s) order by account_head"""
			% ", ".join(["%s"] * len(invoice_list)),
			tuple(inv.name for inv in invoice_list),
		)
		...
		unrealized_profit_loss_accounts = frappe.db.sql_list(
			...
			from `tabSales Invoice` where docstatus and name in (%s)
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql(
		...
		where docstatus %s order by posting_date desc, name desc""".format(
	...
	for d in si_items:
		...
		if d.delivery_note:
			...
			delivery_note_list = frappe.db.sql_list(
				...
				where docstatus and so_detail=%s""",
				d.so_detail,
			)
