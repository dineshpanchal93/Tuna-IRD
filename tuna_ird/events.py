...
def update_sales_invoice_oncancel(doc,method =None):
    frappe.db.set_value("Sales Invoice",doc.name,{"title":"Cancelled","customer_name":"Cancelled","customer":"Cancelled","total_qty":0,"total":0,"grand_total":0,"rounded_total":0,"in_words":"NA","rounding_adjustment":0,"total_taxes_and_charges":0,"outstanding_amount":0})
    frappe.db.commit()
    for i in doc.items:
